    ...
    public function lunas($id)
{
   ...
   public function masuk(Request $request)
    ...
    public function artikel(){
        $artikels = \App\Models\Berita::orderBy('created_at', 'desc')->get();
        return view('admin.artikel', ['artikels' => $artikels]);
    }

    /**
     * Tampilkan halaman berita_layanan untuk admin (mengirim data artikel)
     */
    public function berita_layanan()
    {
        $artikels = \App\Models\Berita::orderBy('created_at', 'desc')->get();
        return view('admin.berita_layanan', ['artikels' => $artikels]);
    }

    /**
     * Simpan artikel dari panel admin (POST /admin/artikel/simpan)
     */
    public function simpanArtikel(Request $request)
    {
        ...
        // prepare slug: if empty or user pasted a full URL, derive from judul or URL path
        ...
        // ensure slug unique: append timestamp suffix when collision
        ...
        if ($request->hasFile('gambar')) {
            ...
            $gambarPath = 'file/img/berita/' . $filename;
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('Admin simpanArtikel error: ' . $e->getMessage(), ['data' => $insert]);
            return back()->withErrors(['db' => 'Gagal menyimpan artikel'])->withInput();
        }

        return back()->with('success', 'Data berita berhasil dimasukkan');
    }

    /**
     * Simpan jenis layanan dari panel admin (POST /admin/jenis_layanan/simpan)
     */
    public function simpanJenisLayanan(Request $request)
    {
        ...
        if ($request->hasFile('gambar')) {
            $file = $request->file('gambar');
            $destination = public_path('file/img/layanan');
            if (! \Illuminate\Support\Facades\File::isDirectory($destination)) {
                \Illuminate\Support\Facades\File::makeDirectory($destination, 0755, true);
            }
            $filename = time() . '_' . \Illuminate\Support\Str::slug(pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME)) . '.' . $file->getClientOriginalExtension();
            $file->move($destination, $filename);
            $gambarPath = 'file/img/layanan/' . $filename;
        }

        $now = now();

        // normalize link: if present and missing scheme, add https://
        ...
        try {
            \Illuminate\Support\Facades\DB::table('jenis_layanans')->insert($insert);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Admin simpanJenisLayanan error: ' . $e->getMessage(), ['data' => $insert]);
            return back()->withErrors(['db' => 'Gagal menyimpan jenis layanan'])->withInput();
        }

        return back()->with('success', 'Jenis layanan berhasil disimpan');
    }
